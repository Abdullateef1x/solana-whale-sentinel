...
export default function AnalyticsPanel({ backtestResults, signals }: Props) {
  const stats = useMemo(() => {
    ...
    const totalPnl = backtestResults.reduce((a, r) => a + r.pnlPct, 0)
    const winRate = backtestResults.length ? wins.length / backtestResults.length : 0
    ...
    const tokenPerf = Array.from(tokenMap.entries())
      .map(([token, data]) => ({ token, ...data, winRate: data.wins / (data.wins + data.losses) }))
    ...
    signals.forEach(s => {
      ...
      t.count++
      if (s.confidence >= 70) t.wins++
    })
    const typePerf = Array.from(typeMap.entries())
      .map(([type, data]) => ({ type, ...data, rate: data.wins / data.count }))
      .sort((a, b) => b.rate - a.rate)

    // Monthly PnL distribution
    const pnlBuckets = { 'Loss >20%': 0, 'Loss 10-20%': 0, 'Loss 0-10%': 0, 'Win 0-10%': 0, 'Win 10-20%': 0, 'Win >20%': 0 }
    backtestResults.forEach(r => {
    ...
    return { wins: wins.length, losses: losses.length, avgWin, avgLoss, totalPnl, winRate, expectancy, tokenPerf, typePerf, pnlBuckets }
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="p-4 space-y-6">
        {/* Summary KPIs */}
        <div>
          <h2 className="text-whale-dim text-xs font-mono font-bold tracking-widest mb-3">SIGNAL BACKTEST PERFORMANCE (50 SIGNALS)</h2>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { label: 'WIN RATE', value: `${(stats.winRate * 100).toFixed(1)}%`, color: stats.winRate > 0.5 ? 'text-whale-green' : 'text-whale-red' },
              { label: 'EXPECTANCY', value: fmt(stats.expectancy), color: stats.expectancy > 0 ? 'text-whale-green' : 'text-whale-red' },
              { label: 'AVG WIN', value: fmt(stats.avgWin), color: 'text-whale-green' },
              { label: 'AVG LOSS', value: fmt(stats.avgLoss), color: 'text-whale-red' },
            ].map(kpi => (
        ...
        {/* Win/Loss breakdown */}
        <div>
          <h3 className="text-whale-dim text-xs font-mono font-bold tracking-widest mb-3">WIN / LOSS DISTRIBUTION</h3>
          <div className="space-y-2">
            {Object.entries(stats.pnlBuckets).map(([label, count]) => {
              const isWin = label.startsWith('Win')
              return (
                <div key={label} className="flex items-center gap-3">
                  <span className={`text-xs font-mono w-28 ${isWin ? 'text-whale-green' : 'text-whale-red'}`}>{label}</span>
                  <div className="flex-1">
                    <Bar value={count} max={backtestResults.length / 3} color={isWin ? 'bg-whale-green' : 'bg-whale-red'} />
                  </div>
        ...
        {/* Token performance */}
        <div>
          <h3 className="text-whale-dim text-xs font-mono font-bold tracking-widest mb-3">PERFORMANCE BY TOKEN</h3>
          <div className="space-y-2">
            {stats.tokenPerf.slice(0, 6).map(tp => (
              <div key={tp.token} className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <Bar value={Math.abs(tp.pnl)} max={100} color={tp.pnl >= 0 ? 'bg-whale-green' : 'bg-whale-red'} />
                </div>
        ...
        <div>
          <h3 className="text-whale-dim text-xs font-mono font-bold tracking-widest mb-3">SIGNAL TYPE EFFECTIVENESS</h3>
          <div className="space-y-2">
            {stats.typePerf.map(tp => (
              <div key={tp.type} className="flex items-center gap-3">
                ...
                <span className="text-xs font-mono text-whale-blue w-12 text-right">{(tp.rate * 100).toFixed(0)}%</span>
                <span className="text-xs font-mono text-whale-dim w-12 text-right">{tp.count} signals</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <h3 className="text-whale-dim text-xs font-mono font-bold tracking-widest mb-3">RECENT SIGNAL OUTCOMES</h3>
          <div className="border border-terminal-border rounded overflow-hidden">
            ...
            {backtestResults.slice(0, 10).map(r => (
              <div key={r.signalId} className={`grid grid-cols-5 gap-2 px-3 py-1.5 border-b border-terminal-border text-xs font-mono ${
